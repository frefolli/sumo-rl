...
class E0FindBestRewardFunction(Experiment):
  ...
  def training(self):
    REWARDS = ['dwt', 'p', 'as', 'ql', 'dql']
    seed = random.randint(0, 10000)
    self.archive.switch(Configuration(agent='ql', observation='default', reward='dwt', partition='mono', self_adaptive=False, dataset='frankestein'))
    for _ in use_iterations(1):
      for reward in REWARDS:
        self.archive.switch(Configuration.Patch(self.archive.config, reward=reward))
