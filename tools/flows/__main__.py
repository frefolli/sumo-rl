...
def main():
	...
	network = sumo_rl.models.flows.Network.Load(network_file)
	...
	if cli_args.random:
		...
		for idx in range(cli_args.random):
			...
			flows = create_random_flow(network.dead_ends, network.edges, duration=cli_args.duration//2, number_of_busy_directions=cli_args.busyness)
		...
		for idx in range(cli_args.complex):
			...
			for _ in range(cli_args.complexity):
				_flows = create_random_flow(network.dead_ends, network.edges, duration=cli_args.duration//2, number_of_busy_directions=cli_args.busyness)
				for flow in _flows:
		...
		if not cli_args.overwrite:
			os.system("rm -rf %s" % (cli_args.output_dir))
		sumo_rl.models.commons.ensure_dir(cli_args.output_dir)
		all_flows = create_all_flows(network.dead_ends, network.edges, duration=cli_args.duration//2, number_of_busy_directions=cli_args.busyness)
